...
IMPORTANT — product ordering within each collection: Products in the search results are sorted newest first. Preserve this ordering as much as possible, but apply styling adjacency — place items that would naturally be used, worn, or displayed together next to each other. For example: a top next to the trousers and shoes that pair with it, or a dinnerware set next to matching serveware and utensils. The goal is newest products first, with items that pair well grouped together.
